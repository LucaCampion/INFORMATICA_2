...
void inserisciRecord(char fileName[], int numRecord)
{
	...
	fp=fopen(fileName,"wb"); //apertura del file passato come parametro in modalità write binary
...
void stampaFile(char fileName[])
{
	...
	fp=fopen(fileName,"rb");	//apertura del file passato come parametro in modalità read binary
	if(fp!=NULL)//controllo che il file possa essere aperto
	{
